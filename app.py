...
def index():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, description, date, location, image FROM events")
    events = cursor.fetchall()
    cursor.close()
    conn.close()
...
def add_event():
    if request.method == 'POST':
        ...
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("INSERT INTO events (name, description, date, location, image) VALUES (%s, %s, %s, %s, %s)", 
                        (name, description, date, location, filename))
            conn.commit()
            cursor.close()
            conn.close()
        except Exception as e:
